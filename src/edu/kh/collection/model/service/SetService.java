...
public class SetService {
	...
	// SET (집합)
	// 1) 순서를 유지하지 않는다. ( == 인덱스가 없음)
	// 2) 중복을 허용하지 않는다. (null 도 중복X, 1개만 저장 가능)
	
	
	//******** Set이 중복을 확인하는 방법********
	// 컬렉션은 객체만 저장 가능 (Set은 컬렉션이기 때문에 객체만 저장)
	// ->객체가 가지고있는 필드값이 모두 같으면 중복으로 판단.
	// --> 이 때 필드값이 모두 같은지 비교하기 위해서 객체에 "equals()" 메소드가 반드시 오버라이딩 되어야 한다.
	
	public void ex1() {
		
		Set<String> set = new HashSet<String>();
		
		// HashSet은 Set의 대표적인 자식클래스입니다
		// 사용조건 1: 저장되는 객체 equals() 오버라이딩 필수
		// 사용조건 2: 저장되는 객체의 hashCode() 오버라이딩 필수
		// String은 굉장히 완성도 높은 클래스로 이미 equals(), hashCode() 오버라이딩 되어 있음
		// 다른 클래스를 만들 때는 equals, hashCode 오버라이딩 필수
		...
		// size(): 저장된 데이터의 객수를 반환
		System.out.println("저장된 데이터의 수: " + set.size());
		
		// remove(String e) : Set에는 인덱스가 없기 때문에 String 이 들어감
						   // Set에 저장된 객체 중 매개변수 e와 필드값이 같은 객체를 제거
		//					+ Hash라는 단어가 포함된 Set이면 hashCode()도 같이 해야함
		
		System.out.println(set.remove("배달의 민족")); // 삭제하면 true / 못 지웠으면 false
		System.out.println(set.remove("당근")); // 못지웠으면 false
		System.out.println(set); // 제거 확인 용도
		
		// List는 인덱스가 있어서 get()로 요소 하나씩 얻어 올 수 있지만
		// Set은 순서가 없어서 저장딘 객체 하나를 얻어 올 수 있는 방법이 없다
		// -> 대신에 Set 전체의 데이터를 하나씩 반복적으로 얻어 올 수 있다
		...
		// set.iterator(): set을 iterator 하나씩 꺼내 갈 수 있는 모양으로 변환
		
		while(it.hasNext()) { // 끝은 모르기 때문에 for가 아닌 while을 사용
			
			//-> hasNext(): 다음 값이 있으면 true를 반환
			// it.next(): 다음 값 (객체)을 얻어와 저장 
	...
	public void ex2() {
		...
		// hashCode() : 필드 값이 다르면 중복되지않는 숫자를 만드는 메소드
		// -> 왜 만들까? 빠른 데이터 검색을 위해서 (객체가 어디에 있는지 찾기 쉬움)
		
		// HashSet() : 중복 없이 데이터 저장(Set)하고 데이터 검색이 빠름 (Hash)
		
		Member mem1 = new Member("user01","password01",30);
		Member mem2 = new Member("user01","password01",30);
		Member mem3 = new Member("user02","password02",34);
		
		System.out.println( mem1 == mem2 ); // 참조형이기때문에 주소 비교
		// 얕은 복사 경우가 아니라면 다 false
		
		if(mem1.getId().equals(mem2.getId())) {
			if(mem1.getPassword().equals(mem2.getPassword())) {
	...
	public void ex3() {
		...
		Member mem1 = new Member("user01","password01",30);
		Member mem2 = new Member("user01","password01",30);
		Member mem3 = new Member("user02","password02",20);
	...
	public void ex4() {
		// wrapper 클래스 : 기본자료형을 객체로 포장하는 클래스
		// - 컬렉션에 기본 자료형 값을 저장하고 싶을 때 사용
		// - 기본 자료형에 없던 추가 기능, 값을 사용하고 싶을 때 사용
		...
		//기본자료형 -> 포장
		Integer w1 = new Integer(iNum); // 삭제선, 오래되어 곧 삭제될 예정인 기능 
		Double w2 = new Double(dNum); 
		
		// Wrapper 클래스 활용
		System.out.println("int 최대값: " + w1.MAX_VALUE);  // static 클스명.스태틱
	...
	public void ex5() {
		...
		//(Integer)  (int -> Integer) 자동포장
									// AutoBoxing
		// w2 와 100은 원래 연산이 안되어야 하지만
		// Integer는 int의 포장형식이라는 것을 JAVA가 인식하고 있음
		// -> 위와 같은 경우 int를 Integer로 AutoBoxing 해준다
		
		System.out.println("w2 + w3 = " + (w2+w3));
		
		// w2 + w3 == 객체+ 객체 ==> 원래 불가능
		// 하지만 Integer는 int의 포장형태라는 것을 Java가 인식하고 있음
		// -> 연산 시 포장을 자동으로 벗겨냄
	...
	public void exlotto() {
		...
		// Set<Integer> lotto = new HashSet<Integer>(); // 중복제거만
		...
		while(lotto.size() < 6) {
			...
			int random = (int)(Math.random() * 45 + 1); // 1 ~ 45 사이 난수 강제형변환
			
			System.out.println(random); //발생 순서 확인
			
			lotto.add(random);			
			// int 값이 자동으로 Integer로 포장 (AutoBoxing) 되어 lotto 추가
		}
